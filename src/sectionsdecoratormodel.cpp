...
{
    if (index < 0 || static_cast<std::size_t>(index) >= m_rowsMetadata.size())
        return;
                                        ...
                                        int first, int last)
{
    ...
    while (static_cast<std::size_t>(sectionIndex) < m_rowsMetadata.size()) {
    ...
    if (static_cast<std::size_t>(sectionIndex) == m_rowsMetadata.size())
        insertNewSection(sectionIndex);
}
